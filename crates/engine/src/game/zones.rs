...
pub(crate) fn apply_zone_exit_cleanup(
    ...
    if from == Zone::Battlefield {
        ...
        // `transient_continuous_effects`. A play/cast permission bounded by
        // `Duration::UntilHostLeavesPlay` lives on the exiled object instead,
        // so it is revoked here at the same lifecycle point — otherwise a card
        // exiled by "you may play that card for as long as [this permanent]
        // remains on the battlefield" stays playable after its host is gone.
